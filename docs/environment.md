...
当前实验阶段已完成候选范围、aging 阈值和成本模型的代表性消融。后续可从操作手册第 7 节继续更大样本主比较，或从第 8 节的已生成成本模型继续跨负载在线验证；不应把此前 `K=128` 的启动 OOM 当作策略性能结果。当前所有已运行策略共用 `configs/a6000.json` 的 Triton/PyTorch backend、BF16 和 TP=1；恢复时如需调整 attention backend、显存比例、并发配置或候选参数，应记录启动日志中的实际解析值，并对同一比较组使用一致配置。
