...
当前已实现四种候选排序策略、上游接入补丁、五类固定轨迹、流式回放、GPU 标定采集与拟合、逐轮分析工具，并完成本地 CPU 正确性检查。远程 A6000 的源码、模型和 Python 环境已准备完成，核心模块导入与 CLI 检查已通过，机制探索、三种子重复、候选范围消融、trace/no-trace 对照、GPU 成本标定、两档速率的 600 请求主比较、aging 成本对照、三类补充 workload 和正常文本观察已执行。主比较的 30 轮中 27 轮为 600/600；3 轮各有 1 个客户端断开；FCFS 原始轮及三次同条件重试均有失败，根因尚未确定。历史逐轮 CSV 和成本标定在 `results/20260910/`，主比较汇总在 `results/20260911/`，补齐重试与正常文本观察在 `results/20260914/`；服务器仍保留全部 `runs/` 原始目录，体积过大未上传；仓库中的 CSV 与服务器说明已分析，逐请求证据待按手册补充。样本规模仍属于项目阶段性实验，结论只覆盖当前模型、硬件、后端和负载设置。

## 文档

- [最新结果分析](results/20260915/analysis.md)：逐种子收益、长等待代价、aging 混杂与证据缺口。
- [实验进度与结论边界](docs/experiment-status.md)：主比较结果、失败边界、补充 workload 和文本观察。
...
已有服务器按 [运行手册](docs/operations-linux.md)第 1 节恢复，先补导出已有运行的轻量证据，无需重跑已完成的 K=128/0 重复。新建环境才执行第 2 节；第 8、9 节规定按当天日期目录整理与回传。
