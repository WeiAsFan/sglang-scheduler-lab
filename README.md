...
当前已实现四种候选排序策略、上游接入补丁、五类固定轨迹、流式回放、GPU 标定采集与拟合、逐轮分析工具，并完成本地 CPU 正确性检查。远程 A6000 的源码、模型和 Python 环境已准备完成，核心模块导入与 CLI 检查已通过，机制探索、三种子重复、候选范围消融、trace/no-trace 对照、GPU 成本标定、两档速率的 600 请求主比较、aging 成本对照、三类补充 workload 和正常文本观察已执行。主比较的 30 轮中 27 轮为 600/600；3 轮各有 1 个瞬时客户端断开，相关原始记录、同条件重试和缺失服务端记录已在日期目录中如实保留。历史逐轮 CSV 和成本标定在 `results/20260910/`，主比较与完整原始包在 `results/20260911/`，补齐重试与正常文本观察在 `results/20260914/`；服务器仍保留全部 `runs/` 原始目录。样本规模仍属于项目阶段性实验，结论只覆盖当前模型、硬件、后端和负载设置。
